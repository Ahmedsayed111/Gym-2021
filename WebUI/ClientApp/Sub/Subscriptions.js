...
(function (Subscriptions) {
    ...
    var Get_IQ_Custmer_Souscription = new Array();
    ...
    var DetailsType_Sous = new Array();
    ...
    Subscriptions.InitalizeComponent = InitalizeComponent;
    function InitalizeControls() {
        debugger;
        ddlSouscription = document.getElementById("ddlSouscription");
        txtType_Sub = document.getElementById("txtType_Sub");
        ...
        btn_cancel_freeze = document.getElementById("btn_cancel_freeze");
        btn_Ok_freeze = document.getElementById("btn_Ok_freeze");
        btnfreeze = document.getElementById("btnfreeze");
        btnRenewal = document.getElementById("btnRenewal");
        ...
        Ajax.Callsync({
            ...
            success: function (d) {
                ...
                if (result.IsSuccess) {
                    ...
                    DetailsType_Sous = DetailsType_Souscription.filter(function (x) { return x.Active == true; });
                }
            }
        });
    }
    function Display() {
        ...
        Ajax.Callsync({
            ...
            success: function (d) {
                var result = d;
                if (result.IsSuccess) {
                    Get_IQ_Custmer_Souscription = result.Response;
                    for (var i = 0; i < Get_IQ_Custmer_Souscription.length; i++) {
                        ...
                        var Num = GetNumDay(Get_IQ_Custmer_Souscription[i].Start_Date, Get_IQ_Custmer_Souscription[i].End_Date);
    ...
    function _SearchBox_Change() {
        ...
        if (searchbutmemreport.value != "") {
            ...
            SearchDetails = Get_IQ_Custmer_Souscription.filter(function (x) { return x.ID_Cust.toString().search(search_1) >= 0 || x.Name_Customer.toLowerCase().search(search_1) >= 0 || x.Phone_Customer.toLowerCase().search(search_1) >= 0 || x.Carte_Dentite.toLowerCase().search(search_1) >= 0; } /*|| x.PortName.toLowerCase().search(search) >= 0*/);
    ...
    function MasterGridDoubleClick() {
        ...
        Selected_Data = Get_IQ_Custmer_Souscription.filter(function (x) { return x.ID_Cust == Number(divMasterGrid.SelectedKey); });
    ...
    function BindGetOperationItemsGridData(ID_Cust) {
        debugger;
        Ajax.Callsync({
            type: "Get",
    ...
    function Disbly_BuildControls(cnt, AllGetStokItemInfo) {
        ...
        var TrDate = DateFormat(AllGetStokItemInfo[cnt].Start_Date_Detais);
    ...
    function Assign() {
        ...
        if (AddNew == true) {
            ...
            var cod = ($('#txtNumber').val() + (Math.floor(Math.random() * 50000)));
            FileUpload(cod);
        ...
        Ajax.Callsync({
            ...
            success: function (d) {
                ...
                if (result.IsSuccess) {
                    ...
                    btnShow_onclick();
                    Selected_Data = new Array();
                    Selected_Data = Get_IQ_Custmer_Souscription.filter(function (x) { return x.ID_Cust == Number(ID_Cust); });
                    ID_Cust = Selected_Data[0].ID_Cust;
                    $("#rowData").removeClass("display_none");
                    $("#divTotalSatistics").removeClass("display_none");
                    DisplayData(Selected_Data);
    ...
    function btnfreeze_onclick() {
        ...
        var date = GetDate();
        var Days = GetNumDay(date, $('#txtEnd_Date_freeze').val());
    ...
    function txtNumDays_freeze_onchange() {
        Ajax.Callsync({
            ...
            success: function (d) {
                var result = d;
                if (result.IsSuccess) {
                    var Date_1 = result.Response;
                    var End_Date = DateFormat(Date_1);
    ...
    function btn_Ok_freeze_onclick() {
        ...
        var Num_days = Number(txtNumDays_freeze.value) + Number($('#Num_Day').val());
        Ajax.Callsync({
            type: "Get",
            url: sys.apiUrl("Subscriptions", "Select_Date_Day"),
            ...
            success: function (d) {
                var result = d;
                if (result.IsSuccess) {
                    var Date_2 = result.Response;
                    var End_Date = DateFormat(Date_2);
    ...
    function btn_cancel_freeze_onclick() {
        var Num_days = Number($('#Num_Day').val()) - (Number($('#txtRemadays_freeze').val()) + 1);
        Ajax.Callsync({
            type: "Get",
            url: sys.apiUrl("Subscriptions", "Select_Date_Day"),
            data: { Num_Day: Number(Num_days) },
            success: function (d) {
                var result = d;
                if (result.IsSuccess) {
                    var Date_3 = result.Response;
                    var End_Date = DateFormat(Date_3);
                    $('#txtEnd_Date').val(End_Date);
                }
            }
        });
        var NumDays_freeze = Number($('#txtNumDays_freeze').val()) - Number($('#txtRemadays_freeze').val());
    ...
    function view_invitation() {
        var sys = new SystemTools();
        sys.FindKey(Modules.Subscriptions, "btnview_invitation", "ID_Cust = " + Number($('#txtNumber').val()), function () {
            var ID_Supplier = SearchGrid.SearchDataGrid.SelectedKey;
        });
    }
    function txtType_Sub_onchange() {
        if (txtType_Sub.value != 'null') {
            var Type_Sub = new Array();
            Type_Sub = DetailsType_Souscription.filter(function (x) { return x.ID_Souscription == Number(txtType_Sub.value); });
            ...
            Ajax.Callsync({
                ...
                success: function (d) {
                    var result = d;
                    if (result.IsSuccess) {
                        var Date_4 = result.Response;
                        var End_Date = DateFormat(Date_4);
                        $('#txtEnd_Date').val(End_Date);
                    }
                }
            });
